...
作为本教程的学习材料，同步撰写了**《大模型与智能体》**，围绕共用基础、大模型、智能体、边界与未来四条主线展开，共 17 章。完整章节列表见 [_meta.yml](_meta.yml)。
...
用 PyTorch 从零搭一个 decoder-only 模型，在中文小语料上预训练并自回归生成。本任务**扩展**实践书 v2「nanoGPT 模型」的带读：加入 BPE、RoPE、KV cache，在中文数据上复现 TinyStories 的"涌现"现象。
...
3. 数据集
   1. SFT：[MOSS-003-sft-data](https://github.com/OpenLMLab/MOSS)（复旦 NLP 组发布的 110 万条中文多轮对话，取 1-5 万子集即可）
   2. DPO：自行在 Hugging Face 寻找中文偏好数据（如 UltraFeedback 翻译版），或先用英文数据集走通流程
   3. **贯通任务五**：同时下载 [moss-003-sft-plugin](https://github.com/OpenLMLab/MOSS)（带工具调用对话的子集），任务三教模型学会工具调用的输出格式，任务五在 Agent 循环里真的去调
4. 实现要求
